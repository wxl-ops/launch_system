...
export default {
  data() {
    return {
      ...
      tableData: [
        {
          empId:1,
          empIdCardNo:"622322199808083656",
          empStaNum:"1001",
          empName: "王大花",
          empJob: "前端",
          xmanpai: "前端项目",
  ...
  methods: {
    ...
    async editRow() {
      ...
      let result = await reqUpdatePositive(this.formData);
    },
    /* 新增一行 */
    async addRow() {
      let result = await reqInserPositive(this.formDataAdd);
    ...
    /* 查询所有数据 */
    async searchData() {
      let result = await reqSearchPositiveAll();
      this.$nextTick(() => {
