  ...
  <div class="wrapper">
    ...
    >
      ...
      <el-form-item label="用工性质">
        <el-input v-model="formInline.empType" placeholder="用工性质"></el-input>
      </el-form-item>
      <el-form-item label="身份证号">
        <el-input v-model="formInline.empIdCardNo" placeholder="身份证号"></el-input>
      </el-form-item>
      <el-form-item label="户口所在地">
        <el-input v-model="formInline.empFamiAddress" placeholder="户口所在地"></el-input>
      </el-form-item>
...
<script>
import { reqSearchPositive,reqDownloadAllPositive,reqDownloadTargetPositive,reqDownloadTimePositive, reqUploadPositive } from "../../../../api";
export default {
  name: "MainSearch",
  props:['targetPage'],
  data() {
  ...
  methods: {
    ...
    async handleExalSuccess(res,file) {
      ...
      let result = await reqUploadPositive(formData)
    },
    /* 查询列表信息 */
    async onSearch() {
      let info = await reqSearchPositive(this.formInline);
      this.$nextTick(() => {
    ...
    async downloadAllOnBoarding() {
      let download = await reqDownloadAllPositive();
    },
    /* 下载当前页 */
    async downloadTargetOnBoarding() {
      let download = await reqDownloadTargetPositive(this.targetPage)
    },
    /* 下载当前日期内的表 */
    async downloadTimeOnBoarding() {
      let download = await reqDownloadTimePositive(this.downTime)
    },
